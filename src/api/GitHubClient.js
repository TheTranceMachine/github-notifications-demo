...
class GitHubClient {
  constructor(token) {
    this.credentials = token !== null && token.length > 0 ? `Bearer ${token}` : null;
  ...
  async getRepositories(options) {
    return await this.octokit.rest.repos.listForAuthenticatedUser(options);
  }

  async getNotifications({ repositories }) {
    // API not supported, return fake notifications instead
    ...
    const fakeData = repositories.map(async (repository) => {
    ...
    return { data: await Promise.all(fakeData) };
  }
}
